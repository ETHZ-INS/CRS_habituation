...
propAct <- ggplot(d, aes(TimePoint, propActivated)) + 
    stat_summary(fun=mean, geom="line", aes(x=as.integer(TimePoint), group=Treatment, colour=Treatment), linewidth=1.2)  + 
  geom_boxplot(aes(fill=Treatment)) + scale_x_discrete() +
    mytheme() + labs(y="Proportion of\nneurons activated") + ylim(0,NA) +
  scale_color_manual(values=cols) + scale_fill_manual(values=cols) +
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) +
  annotate("text", y=0, x=0.5, label=lab, hjust=0, vjust=0, size=3.5)+ xlab(NULL)
...
actADT <- ggplot(d, aes(TimePoint, ADT, fill=Treatment)) + 
  stat_summary(fun=mean, geom="line", aes(x=as.integer(TimePoint), group=Treatment, colour=Treatment), linewidth=1.2)  + 
  geom_boxplot() + scale_x_discrete() +
  mytheme() + labs(y="median ADT score of\nactivated neurons") + ylim(0,NA) +
  scale_color_manual(values=cols) + scale_fill_manual(values=cols) +
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) +
  annotate("text", y=0, x=0.5, label=lab, hjust=0, vjust=0, size=3.5)+ xlab(NULL)
...
pbprop <- ggplot(ag, aes(TimePoint, p.meanAct)) + 
    ...
    geom_boxplot(aes(fill=Treatment)) + facet_wrap(~celltype, nrow=3, scales="free_x") +
    mytheme() + labs(y="Proportion activated (ADT>0.5)") + 
  scale_x_discrete() + scale_color_manual(values=cols) + scale_fill_manual(values=cols) +
    theme(legend.position=c(1, 0), legend.justification=c(1, 0),
          axis.text.x=element_text(angle=90, hjust=1, vjust=0.5))+ xlab(NULL) +
  #geom_text(data=tests, y=0.32, x=0.5, aes(label=lab), size=3, colour="black", hjust=0, vjust=1)
  geom_text(data=tests, y=Inf, x=0.5, aes(label=lab), size=3, colour="black", hjust=0, vjust=1)
```
...
nprop <- ggplot(ag, aes(TimePoint, p.ADT)) + 
    stat_summary(fun=mean, geom="line", linewidth=1.2, aes(x=as.integer(TimePoint), group=Treatment, colour=Treatment)) + 
    geom_boxplot(aes(fill=Treatment)) + facet_wrap(~celltype, nrow=2, scales="free_x") +
    mytheme() + labs(y="Proportion activated (ADT>0.5)") + 
  scale_x_discrete() + scale_color_manual(values=cols) + scale_fill_manual(values=cols) +
    theme(legend.position=c(1, 0), legend.justification=c(1, 0),
          axis.text.x=element_text(angle=90, hjust=1, vjust=0.5))+ xlab(NULL) +
...
plot_grid(
  ...
  nrow=3, rel_heights = c(2.3,4,2.5), scale=c(0.95,0.95,1), labels=c(NA, "D", NA)
)
dev.off()
```
