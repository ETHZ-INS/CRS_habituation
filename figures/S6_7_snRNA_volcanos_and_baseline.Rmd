...
doOneVp <- function(dea, legend=FALSE){
  ggplot(dea, aes(logFC, -log10(ihw), colour=significance)) + geom_point_rast(size=1, raster.dpi = 150) + 
    ...
    ggh4x::facet_grid2(Comparison~cluster_id, axes = TRUE) + mytheme() +
...
```{r, fig.width=15, fig.height=20}
pdf("S6_snRNA_volcanosARS.pdf", width=15, height=20)
pp
dev.off()
```
...
p1 <- ggplot(dea1, aes(logFC, -log10(ihw), colour=significance)) + ggrastr::geom_point_rast(size=1) + 
  ...
  facet_wrap(~cluster_id, nrow=1, scales="free") + mytheme() +
  scale_color_manual(values=setNames(c("darkblue","darkgrey","darkred"), levels(dea$significance))) +
  labs(color="significant", y="-log10(FDR)") + scale_y_sqrt(breaks=c(2,5,10), limits=c(0,5)) +
...
npas4 <- ggplot(d, aes(TimePoint, logcpm)) + 
  ...
  geom_boxplot(aes(fill=Treatment)) + facet_wrap(~feature+celltype2, nrow=2, scales="free_x") + mytheme() + labs(y="Expression log(CPM)") +
...
pdf("S7_snRNA_baselineCRS_and_Npas4.pdf", width=12, height=12)
