...
```{r, fig.height=12, fig.width=10}
plotGenes <- function(d, res2, facets, nrow=1, qR=FALSE, breakTitles=TRUE, ...){
  d2 <- d[d$facet %in% facets,]
  d2$facet <- factor(d2$facet, facets)
  if(breakTitles){
    levels(d2$facet) <- gsub(" in ", " in\n", levels(d2$facet))
    res2$facet <- gsub(" in ", " in\n", res2$facet)
  }
  res2 <- res2[which(res2$facet %in% levels(d2$facet)),]
  ggplot(d2, aes(TimePoint, log2FC)) + 
    stat_summary(fun=mean, geom="line", linewidth=1.2,
                 aes(x=as.integer(TimePoint), group=Treatment, colour=Treatment)) + 
    geom_boxplot(aes(fill=Treatment)) + scale_x_discrete() + mytheme() +
    ...
    theme(..., axis.text.x=element_text(angle=90, hjust=1, vjust=0.5), title=element_text(size=10))
}
...
p1 <- plotGenes(d, res2A, sel1, legend.position="none")
...
res2$sig <- paste0("CRS q=",gsub("e-0", "e-", sprintf("%.0e", res2$ihw)))
p2 <- plotGenes(d, res2, res2$facet, qR=TRUE)
```
...
plot_grid(
  ...
  p1 + theme(title=element_text(size=9.5)), p2 + theme(title=element_text(size=9.5)),
  nrow=3, rel_heights = c(5.1,1.5,1.5), labels="AUTO", scale=0.95
)
dev.off()
```
