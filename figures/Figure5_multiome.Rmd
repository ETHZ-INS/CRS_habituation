...
p0 <- ggplot(d[d$n>1 & d$Treatment=="Handling",], aes(Time, mean, group=celltype, colour=celltype)) + geom_point(size=2) + geom_line(linewidth=1.1) + mytheme() + scale_color_manual(values=cols) + 
...
names(nm) <- nm <- levels(droplevels(pb$celltype2))
bpl <- dplyr::bind_rows(lapply(nm, FUN=function(x){
  se <- pb[,pb$celltype2==x]
  d <- cbind(as.data.frame(colData(se)), mslfc=colMeans((assay(se, "scaledLFC")[up2,,drop=FALSE])))
  agg4Ribb(d, value.var="mslfc", by=c("Treatment","TimePoint"))
}), .id="Celltype")
bpl$Celltype <- factor(bpl$Celltype, levels(pb$celltype2))
cols2 <- metadata(pb)$anno_colors$Treatment
bpl1 <- ggplot(bpl, aes(as.integer(TimePoint), mslfc, fill=Treatment, color=Treatment)) + 
  geom_line(size=0.5) + geom_ribbon(aes(ymin=lower, ymax=upper), alpha=0.4, color=NA) + 
  facet_wrap(~Celltype, nrow=1) + 
  scale_fill_manual(values=cols2) + scale_color_manual(values=cols2) +
  scale_x_continuous(breaks=1:4, labels=levels(pb$TimePoint)) +
  scale_y_continuous(breaks=c()) + labs(x=NULL, y="Response") + 
  mytheme(FALSE) + theme(axis.text.x=element_blank(), legend.position = "none")
...
res3 <- res3[!grepl("Lamp5|Tshz2",res3$celltype),]
...
ggplot(res3, aes(Comparison, gene, size=-log10(ihw), colour=logFC)) + geom_point() + 
  ...
  mytheme(FALSE) + theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1),
                         panel.grid.major.y = element_line(), axis.line.y = element_blank(), axis.ticks=element_blank()) +
  labs(x=NULL, y="Genes significant in more than one neuronal cell type")
```
...
names(tmp2) <- tmp2 <- sort(names(tmp)[-2])
hms <- lapply(tmp2, FUN=function(x)
  sechm(npb[,npb$broadClass==x], tmp[[x]], gaps_at="TimePoint", 
...
hms2 <- lapply(hms, FUN=function(x) grid.grabExpr(draw(x, show_annotation_legend=FALSE, heatmap_legend_side="bottom"),
                                                                 wrap.grobs = TRUE))

bpl <- dplyr::bind_rows(lapply(tmp2, FUN=function(x){
  se <- npb[,npb$broadClass==x]
  d <- cbind(as.data.frame(colData(se)), mslfc=colMeans(abs(assay(se, "scaledLFC")[tmp[[x]],])))
  agg4Ribb(d, value.var="mslfc", by=c("Treatment","TimePoint"))
}), .id="Celltype")
bpl$Celltype <- factor(bpl$Celltype, names(tmp2))
cols2 <- metadata(npb)$anno_colors$Treatment
bpl2 <- ggplot(bpl, aes(as.integer(TimePoint), mslfc, fill=Treatment, color=Treatment)) + 
  geom_line(size=0.5) + geom_ribbon(aes(ymin=lower, ymax=upper), alpha=0.4, color=NA) + 
  facet_wrap(~Celltype, nrow=1) + 
  scale_fill_manual(values=cols2) + scale_color_manual(values=cols2) +
  scale_x_continuous(breaks=1:4, labels=levels(npb$TimePoint)) +
  scale_y_continuous(breaks=c()) + labs(x=NULL, y="Absolute\nresponse") + 
  mytheme(FALSE) + theme(axis.text.x=element_blank())

phm2 <- plot_grid(bpl2, plot_grid(plotlist=hms2, nrow=1), nrow=2, rel_heights=c(1,4))
phm2
```


```{r, fig.width=11, fig.height=16.5}
pdf("Figure5.pdf", width=11, height=16.5)
plot_grid(
  ...
  plot_grid(bpl1 + theme(legend.position = "none"), hm, nrow=2, rel_heights=c(1.1,4)),
  phm2, nrow=4, labels=c(NA,NA,"G","H"), rel_heights=c(1.9,1.9,3.3,3), scale=c(1,1,0.95,0.95) )
dev.off()
```
