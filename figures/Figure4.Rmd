...
ggplot(rda, aes(logFC, -log10(FDR), colour=sig)) + geom_point()
hm <- grid.grabExpr(draw(sechm(rse[,order(rse$Group, rse$Treatment)], row.names(rda)[which(rda$sig)], assayName = "log2FC", gaps_at="TimePoint", top_annotation=c("Group","Sex"), hmcols = c("blue","black","yellow")), merge=FALSE))
```
...
#png("Figure4.png", width=12, height=12, units = "in", res = 300)
plot_grid(
  plot_grid(plot_grid(rvol, hm, nrow=1, scale=0.95, labels="AUTO"),
            plot_grid(gvol, movol, nrow=1, labels=c("C","E"), scale=0.95), moeg, rnacor, nrow=4, rel_heights=c(1,1,0.7,1.3), labels=c(NA,NA,"F","G"), scale=c(0.95,1,0.95,0.95)),
  plot_grid(fkbp5b,Phyhd1,nrow=2,labels=c("Fkbp5", "Phyhd1"), scale=0.95, label_x = 0.5),
  nrow=1, rel_widths = c(5,4), labels=c(NA,"D")
)
dev.off()
```
