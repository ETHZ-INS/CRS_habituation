...
p1 <- ggplot(ora2, aes(log2.enrichment, -log10(FDR), label=Term, alpha=FDR<0.05)) + 
  geom_text(size=3) + coord_cartesian(xlim=c(0,2.15)) + 
    scale_alpha_manual(values = c("FALSE"=0.2, "TRUE"=0.75)) + mytheme() +
...
p3a <- ggplot(m, aes(log2.enrichment.chip, log2.enrichment.motif,
  ...
  geom_text() + coord_cartesian(xlim=c(NA, 1.1)) + mytheme() + 
...
resFkbp5 <- resa[which(resa$gene=="Fkbp5"),]
m <- merge(resFkbp5[resFkbp5$Comparison=="stressAny",c("cluster_id", "ihw")], resFkbp5[resFkbp5$Comparison=="treatAny",c("cluster_id", "ihw")], by="cluster_id")
m$lab <- paste0("ARS q=", format(m$ihw.x, digit=1),"\n",
                "CRS q=", format(m$ihw.y, digit=1))
m$celltype2 <- gsub("/","\n",m$cluster_id,fixed=TRUE)
...
f1 <- ggplot(d, aes(TimePoint, scaledLFC)) + 
  ...
  geom_boxplot(aes(fill=Treatment)) + facet_wrap(~celltype2, ncol=2, scales="free") + mytheme() + labs(y="Fkbp5 scaled log2(FC)", fill="Treatment", color="Treatment") +
  geom_text(data=m, y=Inf, x=0.5, aes(label=lab), size=3, colour="black", hjust=0, vjust=1) +
  scale_color_manual(values=cols) + scale_fill_manual(values=cols) + scale_x_discrete() +
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5), legend.position = c(1, -0.1), legend.justification = c(1, 0),  legend.box.background=element_blank())
f1
```
...
egr1 <- ggplot(d, aes(TimePoint, logcpm)) + 
  stat_summary(fun=mean, geom="line", linewidth=1.2, aes(x=as.integer(TimePoint), group=Treatment, colour=Treatment))  +
  geom_boxplot(aes(fill=Treatment)) + facet_wrap(~celltype2, nrow=1) + mytheme() + labs(y="Egr1 log(CPM)") +
...
grcor <- ggplot(m, aes(logcpm, log2FC)) + geom_smooth(method=MASS::rlm, alpha=0.2) + 
  mytheme() + geom_point(aes(colour=celltype2), size=3) +
...
pdf("Supp_final.pdf", width=11, height=13)
plot_grid(
  #plot_grid(egr1 + theme(legend.position = "bottom"), labels="D", scale=0.95),
  plot_grid(grcor + theme(legend.position="bottom"), f1, nrow=1, scale=0.95, labels=LETTERS[1:2]),
  plot_grid(p1, p2, p3a, labels=LETTERS[3:5], scale=0.95, nrow=1),
  scale=c(1,0.95,0.95), nrow=2, rel_heights = c(4,2)
)
dev.off()
```
