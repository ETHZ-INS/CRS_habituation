...
(p <- ggplot(dea, aes(logFC, -log10(ihw), colour=significance)) + geom_point_rast(size=1) + 
  ggrepel::geom_text_repel(data=dea2, aes(label=gene), size=3, min.segment.length = 0, color="black") +
  facet_wrap(~cluster_id+Comparison, nrow=9, dir="v") + theme_bw() +
...
(p <- ggplot(dea, aes(logCPM, logFC, colour=ihw<0.05)) + geom_point(size=1) + 
  facet_wrap(~cluster_id+Comparison, nrow=9) + theme_bw() +
