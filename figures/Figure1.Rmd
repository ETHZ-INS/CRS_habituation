...
cors <- sapply(split(m,m$TimePoint), FUN=function(x){
  cor(x$logFC.ars, x$logFC.fss)
})
cors <- data.frame(TimePoint=factor(names(cors),levels(m$TimePoint)),
                   cor=paste0("cor=",format(cors,digits=2)))
...
lfcScatter <- ggplot(m, aes(logFC.fss, logFC.ars)) + 
  ...
  theme_bw() + geom_text(x=2, y=-1.6, hjust=1, data=cors, aes(label=cor)) +
